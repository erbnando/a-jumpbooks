...
class WC_Square_Bulk_Sync_Handler {
	...
	public function wc_to_square_ajax() {
		...
		if ( ( 0 === $process ) && $sync_products ) {
			...
			$square_products = $this->connect->get_square_products();

			// extract square product ids so we can cache it
			$square_product_ids = ! empty( $square_products ) ? array_unique( wp_list_pluck( $square_products, 'id' ) ) : array();
		...
		if ( $sync_products && ! empty( $wc_product_ids ) ) {
			...
			if ( is_object( $wc_product ) ) {
				$this->delete_processed_id( $wc_product->id );
			} else {
	...
	public function get_square_inventory() {
		...
		$square_inventory = get_transient( 'wc_square_syncing_square_inventory' );

		if ( false === $square_inventory ) {

			$square_inventory_arr = $this->connect->get_square_inventory();
			$square_inventory = array();

			foreach( $square_inventory_arr as $inventory ) {
				$square_inventory[ $inventory->variation_id ] = $inventory->quantity_on_hand;
			}

			// cache it
			set_transient( 'wc_square_syncing_square_inventory', $square_inventory, apply_filters( 'woocommerce_square_syncing_square_inventory_cache', DAY_IN_SECONDS ) );

		}

		return (array) $square_inventory;
	 ...
	 * @return bool
	 */
	public function send_sync_email( $email, $message = '' ) {
